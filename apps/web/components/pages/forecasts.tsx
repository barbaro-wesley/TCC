...
import { HorizonTabs, Metric, PageHeader, SectionHeader, StatusPill, horizonLabel } from "../dashboard";
...
export function ForecastsPage({ data }: { data: DashboardData }) {
  ...
  return <div className="page-enter">
    ...
    <section className="forecast-summary panel">
      <div className="forecast-number"><span>PREVISÃO {horizonLabel(horizon).toUpperCase()}</span><strong>{brl.format(forecast.point)}<small>/L</small></strong><em className={forecast.changePct>0?"risk-text":"positive-text"}>{percent(forecast.changePct)} vs. preço atual</em></div>
    ...
    <section className="two-col section-gap">
      <div className="panel leaderboard"><SectionHeader title="Leaderboard" subtitle={`Comparação no horizonte de ${horizonLabel(horizon)}`}/>{models.length?<div className="model-table"><div className="table-head"><span>Modelo</span><span>MAE</span><span>MASE</span><span>Direção</span><span>Peso</span></div>{models.map((model,index)=><div className="table-row" key={`${model.model}-${model.horizon}`}><div><b>{index+1}</b><span><strong>{model.model}</strong><small>{model.family}</small></span><StatusPill status={model.status}/></div><span>{model.mae.toFixed(3)}</span><span>{model.mase.toFixed(2)}</span><span>{model.directionalAccuracy.toFixed(1)}%</span><div className="weight-cell"><i style={{width:`${Math.max(2,(model.mae/maxMae)*100)}%`}}/><span>{model.weight}%</span></div></div>)}</div>:<div className="empty-inline">Métricas detalhadas ainda não disponíveis para este horizonte.</div>}</div>
