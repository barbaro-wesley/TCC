...
const STEPS=[
  ...
  {n:"04",title:"Champion por horizonte",body:"1, 2 e 4 semanas (7, 14 e 28 dias) competem separadamente contra naive, médias móveis e modelos estatísticos."},
...
function Limitations(){const items=[{title:"Cobertura histórica curta no varejo",body:"O alvo nacional possui 136 semanas completas, de 07 jan 2024 a 09 ago 2026. Isso ainda limita regimes observados e favorece modelos parcimoniosos."},{title:"Semana corrente ainda é parcial",body:"Coletas de 10–14 ago 2026 pertencem à semana encerrada em 16 ago e permanecem fora do target até o fechamento operacional."},{title:"Preço público não é preço contratado",body:"Economia simulada usa média de revenda; frete, descontos, prazo e take-or-pay ainda não entram no contrafactual."},{title:"Publication lag combina fato e premissa",body:"A semana de 09 ago usa publicação capturada em 14 ago 2026, 14h04 BRT; semanas sem horário comprovado mantêm o fallback conservador de +8 dias."},{title:"Challengers não são reprodução acadêmica",body:"VS-ePL-KRLS permanece experimental e com peso zero até validação matemática e de licença."},{title:"Choques são difíceis por definição",body:"O salto de março concentrou grande parte do erro. Intervalos ajudam a comunicar risco, não eliminam surpresa."}];return <section><div className="limitations-grid">{items.map((item,index)=><article className="panel" key={item.title}><span>LIMITAÇÃO {String(index+1).padStart(2,"0")}</span><h2>{item.title}</h2><p>{item.body}</p><StatusPill status={index<2?"atenção":"documentado"}/></article>)}</div><div className="honesty-note panel"><Icon name="info"/><div><strong>Como ler esta demo</strong><p>O snapshot local usa o target nacional e os artefatos reais persistidos. A API, quando conectada, substitui todo o contrato após validação estrutural.</p></div></div></section>}
